...
//                    console.log('token container no tags');
    ...
    <h3>Artikel</h3>
    <form id="articleForm" action="{{url('article')}}" method="POST">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <div class="form-group label-floating">
            <label class="control-label" for="question_title">Article title</label>
            <input class="form-control" type="text" name="article_title" value="{{$article ?: ''}}" required/>
        </div>
        <div class="form-group label-floating">
            <label class="control-label" for="first_post">Describe your article</label>
        ...
        <div class="form-group right">
            <button type="submit" id="btn-custom" class="btn-block btn btn-primary">uhuy!</button>
        </div>
    </form>
