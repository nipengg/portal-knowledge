    ...
    <script>
        $(document).ready(function () {
            $("#filter-picker").on('change', function(){
                window.location.href = "{{url('/tag/article/'.$theme.'?filter=')}}" + $("#filter-picker").val();
            });
        });
    </script>
    
@if((Session::get('is_approved') === 'active') or (Session::has('username') === false))
<h3>Article theme with: {{$theme}}</h3>
<div>
                ...
                <div class="content">
                    <div class="row">
                        <div class="col-sm-8 col-xs-10">
                            ...
                            <span class="tags">
                                <a href="" class="tag"><span class="label label-info">Theme : {{$article->theme}}</span></a>&nbsp;
                           </span>
                        </div>
                    </div>
                </div>
